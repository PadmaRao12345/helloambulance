...
public class AmbulanceGridController {

	@Autowired
	GridServices gridServices;

	@SuppressWarnings("unchecked")
	@RequestMapping(value = "/ambulance", method = { RequestMethod.GET, RequestMethod.POST })
		...
		try {
			...
			if (JQGridParams.get_search()) {
				...
				for (Rule rule : jQGridFilter.getRules()) {
					if (rule.getField().equals("register_ambulance_id")) {
						searchField = rule.getField();
						searchParam = rule.getData();
					} else if (rule.getField().equals("organization_name")) {
						searchField = rule.getField();
						searchParam = rule.getData();
					} else if (rule.getField().equals("type_of_ambulance")) {
						searchField = rule.getField();
						searchParam = rule.getData();
					} else if (rule.getField().equals("purchesed_from")) {
						searchField = rule.getField();
						searchParam = rule.getData();
					} else if (rule.getField().equals("vechicle_model")) {
						searchField = rule.getField();
						searchParam = rule.getData();
					} else if (rule.getField().equals("vechicle_reg_no")) {
						searchField = rule.getField();
						searchParam = rule.getData();
					} else if (rule.getField().equals("vechicle_engine_no")) {
						searchField = rule.getField();
						searchParam = rule.getData();
					} else if (rule.getField().equals("vechicle_colour")) {
						searchField = rule.getField();
						searchParam = rule.getData();
				...
				map = gridServices.getAmbulanceData(searchField, searchParam,
