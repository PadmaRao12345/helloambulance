...
@Controller
@RequestMapping(value = "/DoctorSpecialization")
public class DoctorSpecializationController {
		...
		try {
			...
			if (JQGridParams.get_search()) {
				...
				for (Rule rule : jQGridFilter.getRules()) {
					if (rule.getField().equals("doctor_specialization_id")) {
						searchField = rule.getField();
						searchParam = rule.getData();
					} else if (rule.getField().equals("doctor_specialization_type")) {
						searchField = rule.getField();
						searchParam = rule.getData();
					} else if (rule.getField().equals("doctor_degree_id")) {
						searchField = rule.getField();
						searchParam = rule.getData();
					}
				}
				...
				map = gridServices.getDoctorSpecializationData(searchField, searchParam,
	...
	public ModelAndView Add(ModelAndView model, @RequestParam String doctor_specialization_type) throws IOException {
		System.out.println(doctor_specialization_type);
		DoctorSpecailization doctorSpecailization = new DoctorSpecailization();
		adminServices.insertDoctorSpecializationType(doctor_specialization_type);
	...
	// Active/InActive Status....
			@RequestMapping(value = "/DoctorDegreeSpecializationStatus")
			public ModelAndView Status(ModelAndView model, @RequestParam String status, @RequestParam String doctor_specialization_id) {
				System.out.println(status);
				DoctorSpecailization doctorSpecialization = new DoctorSpecailization();
				doctorSpecialization.setStatus(status);
				doctorSpecialization.setDoctor_specialization_id(ApplicationUtil.stringToInt(doctor_specialization_id));
				adminServices.updateDoctorDegreeSpecializationstatus(doctorSpecialization);
				model.setViewName("Doctor");
				return model;
			}
