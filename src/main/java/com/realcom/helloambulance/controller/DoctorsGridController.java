...
public class DoctorsGridController {
		...
		try {
			...
			if (JQGridParams.get_search()) {
				...
				for (Rule rule : jQGridFilter.getRules()) {
					if (rule.getField().equals("register_as_doctor_id")) {
						searchField = rule.getField();
						searchParam = rule.getData();
					} else if (rule.getField().equals("doctor_degree")) {
						searchField = rule.getField();
						searchParam = rule.getData();
					} else if (rule.getField().equals("doctor_sprecialization")) {
						searchField = rule.getField();
						searchParam = rule.getData();
					} else if (rule.getField().equals("hospital_name")) {
						searchField = rule.getField();
						searchParam = rule.getData();
					} else if (rule.getField().equals("experience")) {
						searchField = rule.getField();
						searchParam = rule.getData();
					} else if (rule.getField().equals("practice_location")) {
						searchField = rule.getField();
						searchParam = rule.getData();
				...
				map = gridServices.getDoctorsData(searchField, searchParam,
